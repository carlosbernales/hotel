...
// Add this at the beginning of the file, after the require statements
if (!isset($_SESSION)) {
    session_start();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_featured_room'])) {
        try {
            ...
            if (!$stmt->bind_param("isss", $room_type_id, $start_date, $end_date, $image_path)) {
                throw new Exception("Failed to bind parameters: " . $stmt->error);
            }
            
            if (!$stmt->execute()) {
                throw new Exception("Failed to add featured room: " . $stmt->error);
            }
            ...
            echo "<script>
                Swal.fire({
                    icon: 'success',
                    title: 'Success',
                    text: 'Featured room has been added successfully',
                    timer: 1500,
            ...
            if ($success) {
                ...
                echo "<script>
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: 'Failed to delete some discounts'
                    });
                </script>";
            }
        }
