...
if (isset($_POST['action']) && $_POST['action'] == 'early_checkout' && isset($_POST['booking_id'])) {
    ...
    if ($stmt->execute()) {
        ...
        echo "<script>alert('Error processing early check-out: " . $stmt->error . "');</script>";
    }
}
...
if (isset($_POST['action']) && $_POST['action'] == 'archive' && isset($_POST['booking_id'])) {
    ...
    if ($stmt->execute()) {
        ...
        echo "<script>alert('Error archiving booking: " . $stmt->error . "');</script>";
    }
}
...
$room_sql = "SELECT 
    ...
    CASE 
        ...
        WHEN b.payment_option = 'Downpayment' THEN 1500
        ELSE 0
    END as amount_paid,
    CASE 
        WHEN b.payment_option = 'Full' THEN 0
        WHEN b.payment_option = 'Downpayment' THEN (b.total_amount - 1500)
        ELSE b.total_amount
                        ...
                        <div class="col-lg-12">
                            <div class="panel panel-default">
                                <div class="panel-heading">
                                    <h3 class="panel-title">Checked In Room Bookings</h3>
                                </div>
                                <div class="panel-body">
                                                ...
                                                <tbody>
                                                    <?php if (mysqli_num_rows($room_result) > 0): ?>
                                                        <?php while ($row = mysqli_fetch_assoc($room_result)): ?>
                                                            <!-- Debug output -->
                                                            <?php
                                                            error_log("Contact value: " . print_r($row['contact'], true));
                                                            ?>
                                                            <tr>
                                                                ...
                                                                <td class="action-buttons">
                                                                    ...
                                                                    <button class="btn btn-info btn-sm extend-stay-btn"
                                                                            ...
                                                                            data-checkout="<?php echo htmlspecialchars($row['check_out']); ?>"
                                                                            data-room-type="<?php echo htmlspecialchars($row['room_type']); ?>"
                                                                            data-room-price="<?php echo htmlspecialchars($row['room_price']); ?>"
                                                                            data-amount-paid="<?php echo htmlspecialchars($row['amount_paid']); ?>"
                                                                            data-remaining-balance="<?php echo htmlspecialchars($row['remaining_balance']); ?>">
                                                                    ...
                                                                    <button class="btn btn-success btn-sm checkout-btn"
                                                                            ...
                                                                            data-id="<?php echo htmlspecialchars($row['booking_id']); ?>"
                                                                            data-guest="<?php echo htmlspecialchars($row['guest_name']); ?>"
                                                                            data-amount="<?php echo htmlspecialchars($row['total_amount']); ?>"
                                                                            data-paid="<?php echo htmlspecialchars($row['amount_paid']); ?>">
                                                                    ...
                                                                    <button class="btn btn-warning btn-sm early-checkout-btn"
                                                                            ...
                                                                            data-id="<?php echo htmlspecialchars($row['booking_id']); ?>"
                                                                            data-guest="<?php echo htmlspecialchars($row['guest_name']); ?>"
                                                                            data-room-price="<?php echo htmlspecialchars($row['total_amount']); ?>"
                                                                            data-amount-paid="<?php echo htmlspecialchars($row['amount_paid']); ?>">
                                                                    ...
                                                                    <button class="btn btn-primary btn-sm transfer-room-btn"
                                                                            ...
                                                                            data-id="<?php echo htmlspecialchars($row['booking_id']); ?>"
                                                                            data-guest="<?php echo htmlspecialchars($row['guest_name']); ?>"
                                                                            data-current-room="<?php echo htmlspecialchars($row['room_type']); ?>"
                                                                            data-current-roomid="<?php echo htmlspecialchars($row['room_type_id']); ?>"
                                                                            data-current-price="<?php echo htmlspecialchars($row['room_price']); ?>"
                                                                            data-amount-paid="<?php echo htmlspecialchars($row['amount_paid']); ?>"
                                                                            data-remaining-balance="<?php echo htmlspecialchars($row['remaining_balance']); ?>">
            ...
            <form id="earlyCheckoutForm">
                <div class="modal-body">
                    ...
                    <div class="row mt-3">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>Original Number of Nights:</label>
        ...
        <div class="modal-content">
            ...
            <form id="transferRoomForm">
                <div class="modal-body">
                    ...
                    <input type="hidden" id="currentRoomId" name="current_room_id">
                    ...
                    <div class="form-group">
                        <label>Current Room Price</label>
                    ...
                    <div class="form-group">
                        <label>Select New Room <span class="text-danger">*</span></label>
                        <select class="form-control" id="newRoomSelect" name="new_room_id" required>
                            <option value="">Select New Room</option>
                        </select>
                    </div>
                    
                    <div id="priceChangeWarning" class="alert alert-info mt-3" style="display:none;">
                        <div class="price-difference-details">
                            <h6 class="mb-3">Price Adjustment Details:</h6>
                            <div class="row">
                                <div class="col-6">Current Room Rate:</div>
                                <div class="col-6 text-right" id="currentRateDisplay">₱0.00</div>
                            </div>
                            <div class="row">
                                <div class="col-6">New Room Rate:</div>
                                <div class="col-6 text-right" id="newRateDisplay">₱0.00</div>
                            </div>
                            <div class="row">
                                <div class="col-6">Number of Nights:</div>
                                <div class="col-6 text-right" id="nightsDisplay">0</div>
                            </div>
                            <div class="row">
                                <div class="col-6">Amount Already Paid:</div>
                                <div class="col-6 text-right" id="amountPaidDisplay">₱0.00</div>
                            </div>
                            <hr>
                            <div class="row font-weight-bold">
                                <div class="col-6">Final Amount Due:</div>
                                <div class="col-6 text-right" id="finalAmountDisplay">₱0.00</div>
                            </div>
                            <div id="paymentNote" class="mt-2"></div>
                        </div>
                    </div>

                    <div id="paymentSection" class="mt-3" style="display:none;">
                        <div class="form-group">
                            <label>Payment Method for Additional Charge</label>
                        ...
                        <div id="cashPaymentSection" style="display:none;">
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control" id="cashAmount" name="cash_amount" step="0.01">
                            </div>
                            <div class="form-group">
                                <label>Change</label>
                ...
                <div class="card mt-4">
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="info-group mb-3">
                                    <label class="text-muted mb-1">Amount Paid</label>
                                    <div class="h6" id="viewAmountPaid"></div>
                                </div>
                                <div class="info-group mb-3">
            ...
            <form id="extendStayForm">
                <div class="modal-body">
                    ...
                    <div class="row mt-3">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="date" class="form-control" id="newCheckOut" name="new_checkout" required>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group mt-3">
                        ...
                        <select class="form-control" id="extendPaymentMethod" name="payment_method" required>
                            <option value="">Select Payment Method</option>
                            <option value="Cash">Cash</option>
                            <option value="GCash">GCash</option>
                            <option value="Maya">Maya</option>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                    <button type="submit" class="btn btn-info">Confirm Extension</button>
                </div>
            </form>
        </div>
    </div>
...
$(document).ready(function() {
    ...
    $(window).on('resize', function() {
        $('#bookingsTable').DataTable().columns.adjust();
    });
    ...
    $('.checkout-btn').on('click', function(e) {
        ...
        const amountPaid = parseFloat($(this).data('paid'));
        const remainingBalance = totalAmount - amountPaid;
        
        // Populate the checkout modal
        $('#checkoutBookingId').val(bookingId);
        $('#checkoutGuestName').val(guestName);
        $('#checkoutTotalAmount').val('₱' + totalAmount.toFixed(2));
        $('#checkoutPaidAmount').val('₱' + amountPaid.toFixed(2));
    ...
    $('#checkoutForm').on('submit', function(e) {
        ...
        const remainingBalance = parseFloat($('#checkoutRemainingBalance').val().replace('₱', '').replace(/,/g, ''));
    ...
    // Early Check-out Button Handler
    $('.early-checkout-btn').on('click', function(e) {
        e.preventDefault();
        const bookingId = $(this).data('id');
        ...
        const guestName = cells.eq(1).text().trim();

        // Get dates from the table cells
        const checkInDate = new Date(cells.eq(3).text());
        const originalCheckOutDate = new Date(cells.eq(4).text());
        const currentDate = new Date();
        
        // Format dates for display
        const formattedOriginalCheckOut = originalCheckOutDate.toLocaleDateString('en-US', {
            month: 'short',
            day: 'numeric',
            year: 'numeric'
        });
        ...
        // Calculate nights
        const originalNights = parseInt(cells.eq(5).text());
        const actualNights = Math.max(1, Math.ceil((currentDate - checkInDate) / (1000 * 60 * 60 * 24)));

        // Get payment details
        const originalAmount = parseFloat(cells.eq(8).text().replace('₱', '').replace(/,/g, '')); // Total Amount
        const paymentOption = cells.eq(9).text().trim(); // Payment Option column
        const amountPaid = parseFloat(cells.eq(10).text().replace('₱', '').replace(/,/g, '')); // Amount Paid

        // Calculate amounts based on payment option
        let newAmountDue = 0;
        let amountToReturn = 0;

        if (paymentOption.toLowerCase() === 'downpayment') {
            // For downpayment option: Customer pays half of total original amount
            newAmountDue = originalAmount * 0.5;
            // If they've paid more than half, no refund
            amountToReturn = 0;
        } else {
            // For full payment: No refund policy applies
            newAmountDue = originalAmount;
            amountToReturn = 0;
        }

        // Calculate remaining balance
        const remainingBalance = Math.max(0, newAmountDue - amountPaid);

        // Set values in modal
        $('#earlyCheckoutBookingId').val(bookingId);
        $('#earlyCheckoutGuestName').val(guestName);
        $('#originalCheckOutDate').val(formattedOriginalCheckOut);
        $('#newCheckOutDate').val(new Date().toLocaleDateString('en-US', {
            month: 'short',
            day: 'numeric',
            year: 'numeric'
        }));
        $('#originalNights').val(originalNights);
        $('#actualNights').val(actualNights);
        $('#originalAmount').val('₱' + originalAmount.toFixed(2));
        $('#newAmount').val('₱' + newAmountDue.toFixed(2));
        $('#earlyCheckoutPaidAmount').val('₱' + amountPaid.toFixed(2));
        
        // Update modal layout to remove amount to return field and add policy notice
        const policyNotice = paymentOption.toLowerCase() === 'downpayment' 
            ? 'According to our policy, for early check-out with downpayment option, the total amount due is 50% of the original booking amount.'
            : 'According to our policy, for early check-out with full payment, no refund will be provided.';

        // Remove the amount to return row if it exists
        $('#amountToReturnRow').remove();

        // Add policy notice
        if (!$('#policyNoticeRow').length) {
            $('#earlyCheckoutForm .modal-body').append(`
                <div id="policyNoticeRow" class="row mt-3">
                    <div class="col-12">
                        <div class="alert alert-info">
                            <i class="fas fa-info-circle"></i> ${policyNotice}
                        </div>
                    </div>
                </div>
            `);
        ...
        // Show payment section only if there's a remaining balance
        if (remainingBalance > 0) {
            $('#earlyCheckoutPaymentSection').show();
            $('#earlyCheckoutPaymentMethod').prop('required', true);
            // Add remaining balance display
            if (!$('#remainingBalanceRow').length) {
                $('#earlyCheckoutForm .modal-body').append(`
                    <div id="remainingBalanceRow" class="row mt-3">
                        <div class="col-12">
                            <div class="form-group">
                                <label>Remaining Balance to Pay:</label>
                                <input type="text" class="form-control" value="₱${remainingBalance.toFixed(2)}" readonly>
                            </div>
                        </div>
                    </div>
                `);
            ...
            $('#earlyCheckoutPaymentMethod').prop('required', false);
            $('#remainingBalanceRow').remove();
        }
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    ...
                    error: function(xhr, status, error) {
                        ...
                        Swal.fire({
                            title: 'Error!',
                            text: 'Failed to process early check-out. Please try again.',
                            icon: 'error',
                            confirmButtonColor: '#dc3545'
                        });
                        submitBtn.prop('disabled', false).text('Confirm Early Check Out');
    ...
    $(document).on('click', '.view-details', function() {
        ...
        $('#viewAmountPaid').text(amountPaid);
    ...
    // Handle Transfer Room button click
    $('.transfer-room-btn').on('click', function() {
        const bookingId = $(this).data('id');
        const guestName = $(this).data('guest');
        const currentRoom = $(this).data('current-room');
        const currentRoomId = $(this).data('current-roomid');
        const currentRoomPrice = parseFloat($(this).data('current-price'));
        const amountPaid = parseFloat($(this).data('amount-paid')) || 0;
        const remainingBalance = parseFloat($(this).data('remaining-balance')) || 0;
        const row = $(this).closest('tr');
        
        // Get check-in and check-out dates from the row
        const checkInDate = row.find('td:eq(4)').text();
        const checkOutDate = row.find('td:eq(5)').text();
        const nights = parseInt(row.find('td:eq(6)').text()) || 0;
    ...
    $('#newRoomSelect').on('change', function() {
        ...
        if (selectedOption.val()) {
            ...
            // Check if trying to transfer to the same room type
            if (currentRoomId === selectedRoomId) {
                Swal.fire({
                    icon: 'warning',
                    title: 'Same Room Type',
                    text: 'You are selecting the same room type. Please choose a different room type for transfer.',
                    confirmButtonColor: '#3085d6'
                });
            ...
            $('#amountPaidDisplay').text('₱' + amountPaid.toFixed(2));
    ...
    $('#transferRoomForm').on('submit', function(e) {
        ...
        const selectedOption = $('#newRoomSelect').find('option:selected');
        if (!selectedOption.val()) {
            Swal.fire({
                icon: 'error',
        ...
        if (priceDifference > 0) {
            ...
            if (paymentMethod === 'Cash') {
                ...
                if (cashAmount < priceDifference) {
                    Swal.fire({
                        icon: 'error',
                        title: 'Insufficient Amount',
                        text: 'Please enter an amount equal to or greater than the additional charge required'
                    });
                    return false;
                }
            }
        ...
        let confirmationMessage = 'Are you sure you want to proceed with the room transfer?';
        if (priceDifference > 0) {
            confirmationMessage += `\nAdditional payment of ₱${priceDifference.toFixed(2)} is required.`;
        } else if (priceDifference < 0) {
            confirmationMessage += '\nNote: The price difference is non-refundable as per our policy.';
        }
        ...
        Swal.fire({
            ...
            html: `
                <div class="text-left">
                    ...
                    <p><strong>Transfer Reason:</strong> ${$('#transferReason').val()}</p>
                </div>
            `,
            icon: 'question',
            showCancelButton: true,
            confirmButtonText: 'Yes, Transfer Room',
            cancelButtonText: 'Cancel',
            confirmButtonColor: '#3085d6',
            ...
            if (result.isConfirmed) {
                ...
                formData.append('is_refundable', 'false'); // Add non-refundable flag
                    ...
                    success: function(response) {
                        if (response.success) {
                            Swal.fire({
                                icon: 'success',
                                title: 'Success!',
                                text: 'Room transfer completed successfully!'
                            }).then(() => {
                                location.reload();
                            });
                        } else {
                            Swal.fire({
                                icon: 'error',
                                title: 'Error',
                                text: response.message || 'Failed to process room transfer'
                            });
                        }
                    },
                    error: function() {
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
                            text: 'Error processing room transfer'
                        });
                    }
                });
            }
    ...
    $('.extend-stay-btn').on('click', function() {
        ...
        const amountPaid = parseFloat($(this).data('amount-paid'));
    ...
    // Update the new check-out date change handler
    $('#newCheckOut').on('change', function() {
        ...
        const remainingBalance = parseFloat($('#extendRemainingBalance').val().replace('₱', '').replace(/,/g, '')) || 0;
        ...
        const totalAmountToPay = additionalAmount + remainingBalance;
        ...
        // If cash payment is selected, update the minimum cash amount required
        if ($('#extendPaymentMethod').val() === 'Cash') {
            $('#cashAmount').val(totalAmountToPay.toFixed(2));
            $('#changeAmount').val('₱0.00');
        }
    });

    // Update the payment method change handler
    $('#extendPaymentMethod').on('change', function() {
        // No special handling needed anymore
    });

    // Handle extend stay form submission
    $('#extendStayForm').on('submit', function(e) {
        e.preventDefault();
        
        // Get form data
        const bookingId = $('#extendBookingId').val();
        const newCheckOut = $('#newCheckOut').val();
        const additionalNights = $('#additionalNights').val();
        let additionalAmount = $('#additionalAmount').val().replace(/[₱,]/g, '');
        let totalAmountToPay = $('#totalAmountToPay').val().replace(/[₱,]/g, '');
        const paymentMethod = $('#extendPaymentMethod').val();
        ...
        // Convert amounts to numbers and format to 2 decimal places
        ...
        formData.append('total_amount', totalAmountToPay);
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    Swal.fire('Error', response.message || 'Failed to extend stay', 'error');
                }
            },
            error: function(xhr) {
                let errorMessage = 'Failed to extend stay';
                try {
    ...
    // Calculate additional amount when nights change
    $('#additionalNights').on('input', function() {
        const nights = parseInt($(this).val()) || 0;
        const pricePerNight = parseFloat($('#roomPrice').val()) || 0;
        const additionalAmount = nights * pricePerNight;
        ...
        const currentTotal = parseFloat($('#currentTotal').val().replace('₱', '').replace(/,/g, '')) || 0;
        const totalToPay = currentTotal + additionalAmount;
        $('#totalAmountToPay').val('₱' + totalToPay.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}));
    });
