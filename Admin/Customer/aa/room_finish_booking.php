...
try {
    ...
    $downpayment_amount = $payment_option === 'down_payment' ? 1500 : ($payment_option === 'custom_payment' ? floatval($data['amount_paid'] ?? 0) : 0);
    ...
    foreach ($rooms as $room) {
        ...
        $room_quantity = intval($room['quantity'] ?? 1);
        ...
        for ($i = 0; $i < $room_quantity; $i++) {
                ...
                $find_room_sql = "SELECT rn.room_number_id 
                                 ...
                                 AND (br.room_number_fk_id IS NULL 
                                      OR NOT (
                                          ...
                                          AND b.status IN ('accepted', 'checked_in')
                                      ))
                                 LIMIT 1";
            ...
            if (empty($room_number_id)) {
                throw new Exception("No available room found for type: " . $room_type_name . " (Room " . ($i + 1) . " of " . $room_quantity . ")");
            }
            ...
            // Now insert the booking for this specific room
            ...
            error_log("Inserted room: " . $room_type_name . " - Room Number ID: " . $room_number_id . " - Quantity: " . ($i + 1) . "/" . $room_quantity);
        }
    }
    ...
    $notification_sql = "INSERT INTO notifications (
        user_id, booking_fk_id, title, message, type, is_read, created_at
    ) VALUES (
        :user_id, :booking_fk_id, :title, :message, :type, 0, NOW()
    )";
    ...
    try {
        // Prepare booking data for email using RoomBookingMailer format
        $amountPaid = $payment_option === 'down_payment' ? $downpayment_amount : $total_amount;
        ...
        // Send booking confirmation email using RoomBookingMailer
        $roomBookingMailer = new RoomBookingMailer();
        $emailSent = $roomBookingMailer->sendBookingConfirmation($bookingData);
