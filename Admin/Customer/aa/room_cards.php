        ...
        <?php endif; ?>
            ...
            if ($hasSearched) {
                ...
                $imagePath = !empty($room['image']) ? 
                    (strpos($room['image'], 'http') === 0 ? $room['image'] : "../../../Admin/adminBackend/room_type_images/" . basename($room['image'])) : 
                ...
                $bookedStmt = $pdo->prepare("
                    ...
                    AND b.status NOT IN ('completed', 'finished', 'rejected')
                    AND (
                            ...
                            <?php if ($isAvailable): ?>
                                <div class="d-flex flex-column gap-2 align-items-center w-100">
                                    <button class="btn-book w-100 text-center" 
                                            onclick="addToList(<?php echo $room['room_type_id']; ?>, '<?php echo addslashes($room['room_type']); ?>', <?php echo $rawPrice; ?>, <?php echo $room['capacity']; ?>)">
...
function addToList(roomTypeId, roomType, price, capacity) {
    // Store check-in and check-out dates from session to localStorage
    <?php if ($hasSearched): ?>
    localStorage.setItem('checkInDate', '<?= $_SESSION['room_availability']['checkin'] ?>');
    localStorage.setItem('checkOutDate', '<?= $_SESSION['room_availability']['checkout'] ?>');
    <?php endif; ?>
