    ...
    if ($checkin && $checkout && $checkin < $checkout) {
        ...
        $sql = "SELECT 
                ...
                LEFT JOIN (
                    SELECT 
                        rn2.room_type_id,
                        COUNT(br.room_number_fk_id) as booking_count
                    FROM room_numbers rn2
                    LEFT JOIN booked_rooms br ON rn2.room_number_id = br.room_number_fk_id
                    LEFT JOIN bookings b ON br.booking_id = b.booking_id
                    WHERE b.status NOT IN ('completed', 'rejected', 'finished')
                    AND (
                        ...
                        (b.check_in >= ? AND b.check_out <= ?)   -- Existing booking is completely within range
                    )
                    AND b.check_out > ?
...
/* Removed custom .submitbtn CSS as it's now handled by Bootstrap classes */
