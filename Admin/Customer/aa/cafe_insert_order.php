...
try {
    ...
    try {
        ...
        try {
            ...
            $customerName = trim(($orderData['firstname'] ?? '') . ' ' . ($orderData['lastname'] ?? ''));
            
            error_log("Customer email: " . $customerEmail);
            error_log("Customer name: " . $customerName);
            
            if (!empty($customerEmail)) {
                error_log("Attempting to create Mailer instance...");
                $mailer = new Mailer();
                ...
                error_log("Order data prepared for email: " . print_r($emailOrderData, true));
                
                error_log("Attempting to send email to: " . $customerEmail);
                $emailSent = $mailer->sendOrderConfirmation($emailOrderData, $customerEmail, $customerName);
                
                if ($emailSent) {
                    error_log("SUCCESS: Order confirmation email sent successfully to: " . $customerEmail);
                } else {
                    error_log("FAILURE: Failed to send order confirmation email to: " . $customerEmail);
                }
            } else {
                error_log("WARNING: No customer email provided, skipping order confirmation email");
                error_log("Order data received: " . print_r($orderData, true));
            }
        } catch (Exception $e) {
