...
function checkRoomAvailability(roomTypeId, callback) {
    // Get dates from session storage or session
    const checkInDate = localStorage.getItem('checkInDate') || '';
    const checkOutDate = localStorage.getItem('checkOutDate') || '';
    
    // If no dates in localStorage, try to get from session availability
    if (!checkInDate || !checkOutDate) {
        // Try to get from the room availability session
        fetch('get_session_availability.php')
            .then(response => response.json())
            .then(data => {
                if (data.success && data.checkin && data.checkout) {
                    // Store dates for future use
                    localStorage.setItem('checkInDate', data.checkin);
                    localStorage.setItem('checkOutDate', data.checkout);
                    
                    // Check availability with dates
                    checkAvailabilityWithDates(roomTypeId, data.checkin, data.checkout, callback);
                } else {
                    // No dates available, return 0
                    callback(0);
                }
            })
            .catch(error => {
        ...
        // Check availability with stored dates
        checkAvailabilityWithDates(roomTypeId, checkInDate, checkOutDate, callback);
    }
}

function checkAvailabilityWithDates(roomTypeId, checkInDate, checkOutDate, callback) {
    ...
    .then(data => {
        if (data.success && data.rooms && data.rooms.length > 0) {
            // Find the specific room type availability
            const roomData = data.rooms.find(room => room.room_type_id == roomTypeId);
            if (roomData) {
                callback(roomData.available_rooms);
            } else {
                callback(0);
            }
        } else {
            console.error('Error checking room availability:', data.error || 'Unknown error');
            callback(0);
        }
    })
    .catch(error => {
...
function proceedToBooking() {
    ...
    Swal.fire({
                                ...
                                <div class="adult-field mb-3 p-3 border rounded">
                                    ...
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <select class="form-select" id="adult1_usertype" name="adults[0][usertype]" required onchange="handleUserTypeChange(this)">
                                                <option value="" disabled selected>Select user type</option>
                                                <option value="regular">Regular</option>
                                                <option value="senior">Senior Citizen</option>
                                                <option value="pwd">PWD</option>
                                            </select>
                                            <div id="discount-note-1" class="alert alert-warning mt-2" style="display: none; font-size: 0.875rem;">
                                                <i class="fas fa-info-circle me-1"></i>
                                                <strong>Note:</strong> Discount is applicable only on walk-in guests. Please present valid ID upon check-in.
                                            </div>
                                        </div>
                                    </div>
                                </div>
        ...
        showLoaderOnConfirm: true,
        didOpen: () => {
            // Auto-populate dates from room availability search
            populateDatesFromAvailability();
            
            // Set up minimum dates and validation
            setMinDates();
...
function populateDatesFromAvailability() {
    // Get dates from the room availability form
    ...
    if (checkinInput && checkoutInput) {
        ...
        console.log('Found availability dates:', { checkinDate, checkoutDate });
        
        // Set dates in the booking form
        const bookingCheckIn = document.getElementById('checkInDate');
        const bookingCheckOut = document.getElementById('checkOutDate');
        
        if (bookingCheckIn && bookingCheckOut) {
            console.log('Setting booking dates before:', { 
            ...
            console.log('Setting booking dates after:', { 
                checkIn: bookingCheckIn.value, 
                checkOut: bookingCheckOut.value 
            });
            
            // Make the date fields disabled so users cannot modify them
            bookingCheckIn.disabled = true;
            bookingCheckOut.disabled = true;
            
            // Directly call updateBookingSummary since change events might not work on disabled fields
            console.log('Calling updateBookingSummary directly...');
            updateBookingSummary();
            
            console.log('Auto-populated dates:', checkinDate, 'to', checkoutDate);
        }
    }
}
...
function calculateNights(checkIn, checkOut) {
    ...
    console.log('Calculating nights:', { checkIn, checkOut });
    ...
    console.log('After time reset:', { startDate, endDate });
    
    // Calculate the difference in days
    const diffTime = Math.abs(endDate - startDate);
    const diffDays = Math.ceil(diffTime / oneDay);
    
    console.log('Difference calculation:', { diffTime, diffDays });
    
    const result = Math.max(1, diffDays);
    console.log('Final nights result:', result);
...
function updateBookingSummary() {
    ...
    roomPriceElements.forEach(priceElement => {
        ...
        // Get quantity from the booking list data
        const roomType = priceElement.closest('tr').querySelector('td').textContent;
        const bookingList = JSON.parse(localStorage.getItem('bookingList') || '[]');
        const roomData = bookingList.find(room => roomType.includes(room.roomType));
        const quantity = roomData ? parseInt(roomData.quantity) || 1 : 1;
...
function calculateTotalAmount() {
    ...
    roomPriceElements.forEach(priceElement => {
        ...
        const roomRow = priceElement.closest('tr').nextElementSibling; // Get the next row with room total
        const quantityInput = roomRow?.querySelector('.quantity-input');
...
function updateAdultFields() {
    ...
    // Update first adult field number
            ...
            <div class="adult-field mb-3 p-3 border rounded">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" id="adult${i}_usertype" name="adults[${i-1}][usertype]" required onchange="handleUserTypeChange(this)">
                            <option value="" disabled selected>Select user type</option>
                            <option value="regular">Regular</option>
                            <option value="senior">Senior Citizen</option>
                            <option value="pwd">PWD</option>
                        ...
                        <div id="discount-note-${i}" class="alert alert-warning mt-2" style="display: none; font-size: 0.875rem;">
                            <i class="fas fa-info-circle me-1"></i>
                            <strong>Note:</strong> Discount is applicable only on walk-in guests. Please present valid ID upon check-in.
                        </div>
                    </div>
    ...
    // Add event listener to first adult's user type dropdown if it exists
    const firstUserTypeSelect = document.getElementById('adult1_usertype');
    if (firstUserTypeSelect && !firstUserTypeSelect.hasAttribute('data-listener-added')) {
        firstUserTypeSelect.setAttribute('data-listener-added', 'true');
        firstUserTypeSelect.addEventListener('change', function() {
            handleUserTypeChange(this);
        });
    }
}

// Function to handle user type change and show/hide discount note
function handleUserTypeChange(selectElement) {
    const adultIndex = selectElement.id.replace('adult', '').replace('_usertype', '');
    const noteElement = document.getElementById(`discount-note-${adultIndex}`);
    
    if (noteElement) {
        if (selectElement.value === 'senior' || selectElement.value === 'pwd') {
