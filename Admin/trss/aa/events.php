                                ...
                                <div class="card-body d-flex flex-column">
                                    ...
                                    <div class="mt-auto pt-3">
                                        <div class="d-grid gap-2">
                                            ...
                                            <button type="button" class="btn btn-outline-primary btn-details w-100" data-bs-toggle="modal" data-bs-target="#packageDetailsModal" data-package-id="<?php echo $package['id']; ?>">
...
async function checkAvailability() {
    ...
    try {
        ...
        if (result.status === 'success') {
            if (result.data && result.data.status === 'available') {
                ...
                result.data.booked_packages.forEach(packageId => {
                    const packageCard = document.querySelector(`[data-package-id="${packageId}"]`);
                    if (packageCard) {
                        const packageName = packageCard.querySelector('.card-title')?.textContent || `Package #${packageId}`;
            ...
            else if (result.data && result.data.status === 'unavailable') {
        ...
        try {
            ...
            if (isBooked) {
                ...
                if (nextAvailable) {
                    ...
                    badge.innerHTML = '<i class="fas fa-times-circle"></i> Unavailable';
                }
    ...
    function showPackageSections() {
        document.querySelectorAll('.package-section').forEach(section => {
            section.style.display = 'block';
        });
        
        // Scroll to the first package section
