...
$successUrl = $input['success_url'] ?? ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https://" : "http://") . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . '/event_payment_success.php?payment=success&booking_id=' . urlencode($input['metadata']['booking_id'] ?? '') . '&package_name=' . urlencode($input['description']) . '&amount=' . ($input['amount'] / 100) . '&reference=' . $referenceNumber);
$cancelUrl = $input['cancel_url'] ?? (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https://" : "http://") . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . '/event_payment_process.php?payment=cancelled';
        ...
        'attributes' => [
            ...
            'line_items' => [
                [
                    ...
                    'name' => $input['description'] ?? 'Event Booking',
                    'quantity' => 1
                ]
            ],
...
if (strpos($_SERVER['HTTP_HOST'], 'localhost') === false) {
    $checkoutData['data']['attributes']['webhook_url'] = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https://" : "http://") . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . '/event_payment_success.php';
}
...
error_log('Paymongo Request: ' . print_r($checkoutData, true));
