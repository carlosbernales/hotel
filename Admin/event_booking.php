...
// Helper function to generate image carousel HTML
function generateImageCarousel($package) {
    // Get all available images for this package
    $images = [];
    if (!empty($package['image_path'])) $images[] = $package['image_path'];
    if (!empty($package['image_path2'])) $images[] = $package['image_path2'];
    if (!empty($package['image_path3'])) $images[] = $package['image_path3'];
    
    // If no images, use placeholder
    if (empty($images)) {
        $images[] = 'uploads/event_packages/placeholder.jpg';
    }
    ...
    if (count($images) > 1) {
        ...
        $html .= '<div id="carousel-' . $package['id'] . '" class="carousel slide" data-ride="carousel">';
        ...
        foreach ($images as $index => $image) {
            ...
            $html .= 'alt="' . htmlspecialchars($package['name']) . '" ';
            $html .= 'class="img-fluid w-100" ';
            $html .= 'style="height: 250px; object-fit: cover;">';
            $html .= '</div>';
        }
        
        $html .= '</div>';
        $html .= '<a class="carousel-control-prev" href="#carousel-' . $package['id'] . '" role="button" data-slide="prev">';
        $html .= '<span class="carousel-control-prev-icon" aria-hidden="true"></span>';
        $html .= '<span class="sr-only">Previous</span>';
        $html .= '</a>';
        $html .= '<a class="carousel-control-next" href="#carousel-' . $package['id'] . '" role="button" data-slide="next">';
        ...
        $html .= '<img src="' . htmlspecialchars($images[0]) . '" ';
        $html .= 'alt="' . htmlspecialchars($package['name']) . '" ';
        $html .= 'class="img-fluid w-100" ';
        $html .= 'style="height: 250px; object-fit: cover;">';
    }
...
if ($result && $result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        // Debug: Show raw database row
        echo "<!-- Raw DB row for " . $row['name'] . ": " . json_encode($row) . " -->";
        // Handle image path with proper fallback
        $image_path = '';
        
        // Try to use the primary image first
        if (!empty($row['image_path']) && file_exists($row['image_path'])) {
            $image_path = $row['image_path'];
        }
        // If primary image doesn't exist, try secondary image
        elseif (!empty($row['image_path2']) && file_exists($row['image_path2'])) {
            $image_path = $row['image_path2'];
        }
        // If secondary image doesn't exist, try third image
        elseif (!empty($row['image_path3']) && file_exists($row['image_path3'])) {
            $image_path = $row['image_path3'];
        }
        // If no images exist, use a generic placeholder
        else {
            $image_path = 'uploads/event_packages/placeholder.jpg'; // Generic placeholder
        }
...
if (empty($packages)) {
    $packages = [
        ...
        [
            ...
            'image_path' => 'uploads/event_packages/placeholder.jpg',
            'image_path2' => '',
            'image_path3' => ''
        ],
        [
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_booking'])) {
    ...
    if ($customer_name === '' || $customer_name === '0') {
        $_SESSION['error_message'] = "Customer name is required and cannot be empty or zero.";
...
<head>
    ...
    <style>
        ...
        /* Modal carousel specific styling */
...
<body>
                ...
                <div class="row">
                    ...
                    foreach ($packages as $package):
                        if ($package['name'] == 'Venue Rental Only'):
                            // Debug: Show package data
                            echo "<!-- Package data for " . $package['name'] . ": " . json_encode($package) . " -->";
                    ?>
                    <div class="col-md-4">
                        <div class="package-card">
                            <div class="package-image">
                                <?php echo generateImageCarousel($package); ?>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="POST" action="process_event_booking.php" id="bookingForm">
                        ...
                        <div class="form-row mt-3">
                            <div class="form-group col-md-6">
                                <label class="required-field">Amount Paid by Customer</label>
                                <input type="number" class="form-control" name="paid_amount" id="paid_amount" required min="0" step="0.01" placeholder="0.00">
                                <small class="form-text text-muted">Enter the amount the customer is paying now</small>
                            </div>
                            <div class="form-group col-md-6">
                                <label>Remaining Balance</label>
                                <input type="number" class="form-control" name="remaining_balance" id="remaining_balance" readonly style="background-color: #e9ecef;" value="0.00">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="modal-image-container mb-3">
                        <div id="modalImageCarousel" class="carousel slide" data-ride="carousel">
    ...
    function viewPackageDetails(package) {
        ...
        try {
            ...
            const images = [];
            console.log('Package data:', package); // Debug log
            if (package.image_path) images.push(package.image_path);
            if (package.image_path2) images.push(package.image_path2);
            if (package.image_path3) images.push(package.image_path3);
            
            console.log('Found images:', images); // Debug log
            
            // If no images, use placeholder
            if (images.length === 0) {
                images.push('uploads/event_packages/placeholder.jpg');
            }
            ...
            images.forEach((image, index) => {
                ...
                const carouselItem = `
                    <div class="carousel-item ${activeClass}">
                        <img src="${image}" alt="${package.name}" class="modal-image" 
                             onerror="this.src='uploads/event_packages/placeholder.jpg'">
                    </div>
                `;
            ...
            const carouselControls = $('#modalImageCarousel .carousel-control-prev, #modalImageCarousel .carousel-control-next');
            if (images.length > 1) {
                carouselControls.show();
                console.log('Showing carousel controls for', images.length, 'images');
                // Reinitialize carousel for multiple images
                $('#modalImageCarousel').carousel();
            } else {
                carouselControls.hide();
                console.log('Hiding carousel controls for single image');
            }
    ...
    $(document).ready(function() {
        ...
        $('#paid_amount').on('input', function() {
            ...
            const totalAmount = parseFloat($('#total_amount').text().replace(/[^0-9.-]+/g,"")) || 0;
        ...
        function updateFeeSummaryWithOvertime(packagePrice, additionalGuests, overtimeHours, overtimeCharge) {
            ...
            // Update remaining balance if paid amount is already entered
        ...
        $('#confirmEventBooking').on('click', function() {
            ...
            const formData = {
                ...
                remaining_balance: parseFloat($('#remaining_balance').val()) || 0,
