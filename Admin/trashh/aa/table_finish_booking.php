...
if (!empty($_SESSION['user_id'])) {
    $userStmt = $pdo->prepare("SELECT first_name, last_name, contact_number, email FROM userss WHERE id = :user_id");
    $userStmt->execute([':user_id' => $_SESSION['user_id']]);
    $dbUser = $userStmt->fetch(PDO::FETCH_ASSOC);
    
    if ($dbUser) {
        // Use database values as fallback if not provided in $data
        $userDetails['firstname'] = $userDetails['firstname'] ?: ($dbUser['first_name'] ?? '');
        $userDetails['lastname'] = $userDetails['lastname'] ?: ($dbUser['last_name'] ?? '');
        $userDetails['contact'] = $userDetails['contact'] ?: ($dbUser['contact_number'] ?? '');
        $userDetails['email'] = $userDetails['email'] ?: ($dbUser['email'] ?? '');
    }
}
