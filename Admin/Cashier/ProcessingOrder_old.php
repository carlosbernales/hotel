...
<?php
// Modify the SQL query to properly join with userss table and fetch processor name
$sql = "SELECT 
    orders.*,
    ...
    COALESCE(NULLIF(orders.customer_name, ''), 'N/A') as customer_name,
    COALESCE(
        (SELECT CONCAT(u.first_name, ' ', u.last_name, 
            CASE WHEN orders.order_type = 'advance' THEN ' (Advance Order)' ELSE '' END)
         FROM userss u 
         WHERE u.id = orders.user_id),
        CASE WHEN orders.order_type = 'advance' THEN 'Advance Order - Pending Processing' ELSE 'Not processed yet' END
    ) as processed_by,
    ...
    orders.table_name,
    (SELECT u.first_name FROM userss u WHERE u.id = orders.user_id) as processor_first_name,
    (SELECT u.last_name FROM userss u WHERE u.id = orders.user_id) as processor_last_name,
...
FROM orders 
LEFT JOIN userss processor ON orders.user_id = processor.id
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if (!isset($orders[$orderId])) {
            ...
            $orders[$orderId] = [
                ...
                'processed_by' => $row['processor_first_name'] && $row['processor_last_name'] ? 
                    $row['processor_first_name'] . ' ' . $row['processor_last_name'] : 
                    'Not processed yet',
                'items' => [],
            ];
        }
                    ...
                    <table class="table table-striped table-bordered table-responsive" cellspacing="0" width="100%"
                        ...
                        <?php foreach ($orders as $orderId => $order): ?>
                            <tr data-order-id="<?php echo $orderId; ?>">
                                ...
                                <td><?php echo htmlspecialchars($order['processed_by']); ?></td>
                                <td>
                                    <div class="btn-group">
