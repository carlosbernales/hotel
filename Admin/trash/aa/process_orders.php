...
function processRegularOrder($orderData, $pdo) {
    ...
    try {
        ...
        // Insert the order
        $orderSql = "INSERT INTO orders_table (
            user_id, total_amount, extra_fee, order_type,
            payment_method, payment_status, status,
            final_total, order_date, remaining_balance
        ) VALUES (
            :user_id, :total_amount, :extra_fee, :order_type,
            :payment_method, :payment_status, :status,
            :final_total, NOW(),  :remaining_balance
        )";
...
function insertOrderItems($orderId, $items, $pdo) {
    foreach ($items as $item) {
        $itemSql = "INSERT INTO order_items (order_fk_id, item_name, quantity, unit_price) 
