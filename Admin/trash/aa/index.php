...
<head>
    ...
    <style>
        ...
        /* Add padding to body to account for fixed navbar and announcement bar */
        body {
            padding-top: 110px; /* Navbar height (60px) + Announcement bar height (50px) */
        }
...
<body>
    ...
    $announcements = [];
    $defaultBgColor = '#f8f9fa';
    $defaultTextColor = '#495057';
    $defaultSpeed = 10; // Default animation speed in seconds per announcement
    ...
    try {
        ...
        // First, check if the announcement table exists
        $tableCheck = $pdo->query("SHOW TABLES LIKE 'announcement'");
        if ($tableCheck->rowCount() == 0) {
            throw new PDOException('Announcement table does not exist');
        }
        
        // Get current date in the correct format
        $currentDate = date('Y-m-d H:i:s');
        
        // Simple query to check if we can fetch any announcements
        $sql = "SELECT * FROM announcement LIMIT 1";
        $testQuery = $pdo->query($sql);
        if ($testQuery === false) {
            throw new PDOException('Error executing query: ' . implode(', ', $pdo->errorInfo()));
        }
        
        // First, get any active announcements regardless of date
        // We'll handle date filtering in PHP to better debug any issues
        $sql = "SELECT message, text_color, background_color, speed, start_date, end_date 
                FROM announcement 
                WHERE is_active = 1 
        ...
        // Filter announcements by date in PHP for better debugging
        $filteredAnnouncements = [];
        foreach ($announcements as $announcement) {
            $startDateValid = empty($announcement['start_date']) || strtotime($announcement['start_date']) <= time();
            $endDateValid = empty($announcement['end_date']) || strtotime($announcement['end_date']) >= time();
        ...
        // If no announcements within date range, show all active announcements
        $announcements = !empty($filteredAnnouncements) ? $filteredAnnouncements : $announcements;
        
        // Debug: Output the SQL query and results
        // Remove this in production
        // echo "<!-- SQL: " . $sql . " -->";
        // echo "<!-- Announcements: " . print_r($announcements, true) . " -->";
        
        // If no active announcements, use default
        if (empty($announcements)) {
            throw new PDOException('No active announcements found in the database');
        }
        
        // Get the first announcement's styling (assuming all should have same style)
        $announcementStyle = $announcements[0];
        $bgColor = !empty($announcementStyle['background_color']) ? $announcementStyle['background_color'] : $defaultBgColor;
        $textColor = !empty($announcementStyle['text_color']) ? $announcementStyle['text_color'] : $defaultTextColor;
        $animationSpeed = !empty($announcementStyle['speed']) ? (int)$announcementStyle['speed'] : $defaultSpeed;
        
    } catch (PDOException $e) {
        // Fallback to a single default announcement if there's an error
        $announcements = [
            ['message' => "Welcome to Casa Estela - Your Home Away From Home"]
        ];
    ...
    // Only show announcement bar if there are announcements
    if (!empty($announcements)): 
        // Calculate total animation duration based on number of announcements and speed
        $totalDuration = count($announcements) * $animationSpeed;
    ?>
    <!-- Announcement Marquee -->
    <div class="announcement-bar" style="background-color: <?php echo htmlspecialchars($bgColor); ?>; color: <?php echo htmlspecialchars($textColor); ?>; padding: 8px 0; border-bottom: 1px solid #e9ecef; margin-top: 56px;">
        <div class="container">
            <div class="d-flex align-items-center">
                <div class="announcement-icon me-3" style="color: #d4af37;">
                    <i class="fas fa-bullhorn"></i>
                </div>
                <div class="announcement-content" style="flex: 1; overflow: hidden;">
                    <div class="marquee" style="white-space: nowrap; animation: marquee <?php echo $totalDuration; ?>s linear infinite;">
                        <?php foreach ($announcements as $index => $announcement): 
                            $message = is_array($announcement) ? $announcement['message'] : $announcement;
                        ?>
                            <span class="me-5" style="color: <?php echo htmlspecialchars($textColor); ?>"><?php echo htmlspecialchars($message); ?></span>
                        <?php endforeach; ?>
                    </div>
                </div>
            </div>
