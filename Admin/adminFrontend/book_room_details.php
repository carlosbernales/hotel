            ...
            .then(data => {
                document.querySelectorAll('.roomTypeSelect').forEach(select => {
                    select.querySelectorAll('option').forEach(option => {
                        ...
                        // Reset: enable option and remove any "(Occupied/Maintenance)" text
                        option.disabled = false;
                        option.text = option.text.replace(/\s*\(.*\)$/, '');

                        // Disable if there is a conflict
...
<script>
    function updateTotal() {
        ...
        // Subtract down payment to get total due
        ...
        // Update change if payment input already entered
    ...
    document.querySelectorAll('.roomTypeSelect').forEach(select => {
        select.addEventListener('change', function () {
            ...
            let capacity = this.selectedOptions[0].dataset.capacity;
