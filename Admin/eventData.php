...
$users_structure_sql = "DESCRIBE userss";
...
$sql = "SELECT eb.*, 
               eb.package_name as display_package_name,
               ep.menu_items as package_menu,
               CASE 
                   WHEN eb.booking_source = 'frontdesk' THEN eb.customer_name
                   WHEN u.first_name IS NOT NULL THEN CONCAT(u.first_name, ' ', u.last_name)
                   ELSE eb.customer_name
               END as display_customer_name,
               CASE 
                   WHEN eb.reservation_date = '0000-00-00' OR eb.reservation_date IS NULL THEN eb.event_date 
                   ELSE eb.reservation_date 
               END as event_display_date
...
if ($result) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        error_log("Customer Name: " . (isset($row['display_customer_name']) ? $row['display_customer_name'] : 'N/A'));
        error_log("Event Date: " . (isset($row['event_display_date']) ? $row['event_display_date'] : 'N/A'));
        error_log("-------------------");
    }
                                ...
                                <?php if (mysqli_num_rows($result) > 0): ?>
                                    <?php while ($row = mysqli_fetch_assoc($result)): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <?php echo htmlspecialchars($row['display_customer_name'] ?? ''); ?>
                                                <?php if ($row['booking_source'] == 'frontdesk'): ?>
                                                    <span class="badge badge-info">Frontdesk</span>
                                                <?php elseif ($row['booking_source'] == 'Regular Booking'): ?>
                                                    <span class="badge badge-primary">Web</span>
                                                <?php endif; ?>
                                            </td>
                                            <td><?php echo date('M j, Y', strtotime($row['event_display_date'])); ?></td>
                                            ...
                                            <td class="action-buttons">
                                                ...
                                                <?php if ($row['booking_status'] == 'confirmed'): ?>
                                                    <button class="btn btn-success btn-sm"
                                                            data-toggle="tooltip"
                                                            title="Mark as Done"
                                                            onclick="markAsDone('<?php echo $row['id']; ?>')">
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div id="modalErrorState" class="alert alert-danger d-none" role="alert" style="display: none !important;">
...
$(document).ready(function() {
    ...
    $('.view-payment-proof').on('click', function() {
        ...
        proofUrl = proofUrl.split('/').pop();
        
        // Use an absolute path from the webroot assuming 'Admin' is the root directory alias
        var imagePath = '/Admin/uploads/payment_proofs/' + proofUrl;
        ...
        // Set the image source using the determined path
        imgElement.attr('src', imagePath)
            .on('error', function() {
                // Show error message if image fails to load
                $('#paymentProofImage').replaceWith(
                    '<div class="alert alert-danger">' +
                    'Error loading image. Please check if the file exists and the path is correct.<br>' +
                    'Filename: ' + proofUrl + '<br>' +
                    'Attempted path: ' + imagePath +
                    '</div>'
                );
            })
            .on('load', function() {
                // Image loaded successfully
                $('#paymentProofImage').replaceWith(imgElement);
            });

        // Replace the old image element with the new one (initially empty src)
        $('#paymentProofImage').replaceWith(imgElement);
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success && response.details) {
                    // Hide error message when data loads successfully
                    $('#modalErrorState').hide();
                    $('#modalErrorState').css('display', 'none !important');
                    $('#modalErrorState').attr('style', 'display: none !important');
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        Swal.fire('Success', 'Event marked as completed successfully', 'success')
                            .then(() => location.reload());
                    } else {
                        Swal.fire('Error', response.message || 'Failed to mark event as completed', 'error');
                    }
                },
                error: function() {
                    Swal.fire('Error', 'Failed to process request', 'error');
