...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($id) {
            // Check if package name already exists for a different ID
            $check_sql = "SELECT id FROM table_packages WHERE package_name = ? AND id != ?";
            ...
            // Execute the update
            if (!$stmt->execute()) {
                throw new Exception("Error updating package: " . $stmt->error);
            }
        } else {
            // Check if package name already exists
            $check_sql = "SELECT id FROM table_packages WHERE package_name = ?";
            $check_stmt = $con->prepare($check_sql);
