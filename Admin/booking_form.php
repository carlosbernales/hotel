...
if (isset($_GET['room_id'])) {
    ...
    // Hardcoded room data with room_type_id mapping
    $rooms = [
        '101' => [
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" value="<?php echo htmlspecialchars($room['room_type']); ?>" readonly>
                        <input type="hidden" name="room_type" value="<?php echo htmlspecialchars($room['room_type']); ?>">
                        <input type="hidden" name="room_type_id" value="<?php echo (int)$room['room_type_id']; ?>">
                        <?php 
                        // Debug: Log the room type ID being set
                        error_log("Setting room_type_id to: " . $room['room_type_id'] . " for room type: " . $room['room_type']);
                        ?>
                    </div>
                </div>
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" id="extraBed" name="extra_bed" value="1">
                ...
                <div class="col-md-12">
                    <div class="card bg-light mb-3">
                        <div class="card-body">
                            ...
                            <p class="card-text" id="extra_bed_charge_text" style="display: none;">Extra Bed Charge: ₱<span id="extra_bed_charge">0.00</span></p>
                            ...
                            <input type="hidden" name="extra_charges" id="extra_charges_input" value="0.00">
...
$(document).ready(function() {
    ...
    function calculateTotal() {
        ...
        var extraBedCharge = $('#extraBed').is(':checked') ? 1000 : 0;
        
        if (checkIn && checkOut && checkOut > checkIn) {
            var nights = Math.ceil((checkOut - checkIn) / (1000 * 60 * 60 * 24));
            var total = (nights * pricePerNight) + extraBedCharge;
            var remainingBalance = 0;
            
            $('#total_nights').text(nights);
            $('#extra_bed_charge').text(extraBedCharge.toFixed(2));
            $('#extra_bed_charge_text').toggle(extraBedCharge > 0);
            
            $('#total_amount').text(total.toFixed(2));
            $('#total_amount_input').val(total.toFixed(2));
            $('#extra_charges_input').val(extraBedCharge.toFixed(2));
            ...
            $('#total_nights').text(0);
            $('#extra_bed_charge').text('0.00');
            $('#extra_bed_charge_text').hide();
            $('#total_amount').text('0.00');
            $('#total_amount_input').val('0.00');
            $('#extra_charges_input').val('0.00');
            $('#downpayment_text').hide();
            $('#remaining_balance_text').hide();
        }
    }

    // Calculate total when dates or extra bed option changes
    $('input[name="check_in"], input[name="check_out"], #extraBed').change(calculateTotal);
