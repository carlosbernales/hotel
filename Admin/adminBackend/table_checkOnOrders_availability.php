...
while ($row = $resultEvents->fetch_assoc()) {
    $unavailablePlaces[] = $row['place'];
    $existingBookings[] = [
        'place' => $row['place'],
        'booked_time' => date('g:i A', strtotime($row['date_time_start'])) . ' - ' . date('g:i A', strtotime($row['date_time_end']))
    ];
}

if (!empty($unavailablePlaces)) {
    echo json_encode([
        'conflict' => true,
        'places' => $unavailablePlaces, // array of unavailable places
        'bookings' => $existingBookings,
        'message' => 'Some places are not available at this time.'
    ]);
    exit;
}
...
echo json_encode([
    'conflict' => false,
    'message' => 'Time slot available for all places.'
]);
exit;
?>