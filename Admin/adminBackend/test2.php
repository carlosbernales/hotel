                                    ...
                                    <select class="form-select roomNumberSelect">
                                        ...
                                        if ($r['room_number_fk_id'] && isset($roomNumbers[$bookedRoomTypeId])):
                                            foreach ($roomNumbers[$bookedRoomTypeId] as $rn):
                                                $selectedRN = ($rn['id'] == $r['room_number_fk_id']) ? 'selected' : '';
                                                ?>
                                                <option value="<?= $rn['id'] ?>" <?= $selectedRN ?>>
                                                    <?= $rn['room_number'] ?>
                                                </option>
                                            <?php endforeach;
                                        else: ?>
...
<script>
    // Function to update room numbers for a row based on its current room type
    function updateRoomNumbers(row) {
        ...
        fetch(`../Admin/adminBackend/check_availability_rooms.php?check_in=${checkIn}&check_out=${checkOut}&booking_id=${bookingId}`)
            ...
            .then(data => {
                ...
                if (data[selectedType] && data[selectedType].length > 0) {
                    data[selectedType].forEach(rn => {
                        const option = document.createElement('option');
                        option.value = rn.room_number_id;
                        option.textContent = rn.room_number;
                        if (rn.note) {
                            option.textContent += ` (${rn.note})`;
                        }
            ...
            .catch(err => {
                ...
                roomNumberSelect.innerHTML = '<option disabled>Error loading rooms</option>';
            });
    }

    // On page load, populate all room numbers based on preselected room types
    document.addEventListener('DOMContentLoaded', () => {
        document.querySelectorAll('#roomsTable tbody tr').forEach(row => updateRoomNumbers(row));

        // Also attach change listeners
        document.querySelectorAll('.roomTypeSelect').forEach(select => {
            select.addEventListener('change', function () {
                ...
                // Update price and capacity dynamically
    ...
    function checkCapacity() {
        ...
        if (totalCapacity < numberGuest) {
            ...
            // Reset to default PHP-rendered values
            rows.forEach(row => {
                const roomTypeSelect = row.querySelector('.roomTypeSelect');
                ...
                // Update price and capacity for the default option
                ...
                // Reset room number select to default
                updateRoomNumbers(row);
    ...
    // Call this function whenever you need to validate, e.g., on roomTypeSelect change or on booking process
    document.querySelectorAll('.roomTypeSelect').forEach(select => {
        select.addEventListener('change', function () {
            const row = this.closest('tr');
    ...
    function calculateTotalAmount() {
        ...
        // Extra bed price (embedded server-side output)
