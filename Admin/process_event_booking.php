...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Set default values
        $booking_source = 'frontdesk';  // Changed from 'walk_in' to 'frontdesk'
        $booking_status = 'pending';    // Set default booking status

        // Get user ID from session
        $user_id = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 0;
        error_log("Using user_id: " . $user_id . " for frontdesk booking");
        ...
        // Instead of using prepared statements with bind_param (which is causing issues),
        // let's use direct query with proper escaping
        $id = $con->real_escape_string($unique_id);
        $customerName = $con->real_escape_string($customer_name);
        $packageName = $con->real_escape_string($package_name);
        $packagePrice = floatval($base_price);
        $eventDate = $con->real_escape_string($_POST['event_date']);
        $basePrice = floatval($base_price);
        $overtimeHours = intval($overtime_hours);
        $overtimeCharge = floatval($overtime_charge);
        $extraGuests = intval($extra_guests);
        $extraGuestCharge = floatval($extra_guest_charge);
        $totalAmount = floatval($total_amount);
        $paidAmount = floatval($paid_amount);
        $remainingBalance = floatval($remaining_balance);
        $eventType = $con->real_escape_string($_POST['event_type']);
        $startTime = $con->real_escape_string($_POST['start_time']);
        $endTime = $con->real_escape_string($_POST['end_time']);
        $numberOfGuests = intval($number_of_guests);
        $paymentMethod = $con->real_escape_string($_POST['payment_method']);
        $paymentType = $con->real_escape_string($_POST['payment_type']);
        $referenceNumber = $con->real_escape_string('');
        $bookingStatus = $con->real_escape_string('pending');
        $reserveType = $con->real_escape_string('Regular');
        $bookingSource = $con->real_escape_string('admin');

        // Insert without prepared statement
        ...
        // Execute the query
        if (!$con->query($sql)) {
            error_log("Execute failed: " . $con->error);
        ...
        $verify_sql = "SELECT customer_name, package_name FROM event_bookings WHERE id = '$id'";
